...
export default function Step1Inspection({ initialData = {}, onNext }) {
  ...
  const validate = () => {
    ...
    if (!file) {
      errs.file = "Subí tu certificado de VTV (foto o PDF)";
    }
    if (!expiresAt) {
  ...
  const handleNext = () => {
    if (!validate()) return;
    // prepare data to parent
    onNext({ docFile: file, docUrl: previewUrl, expiresAt });
  };

  return (
    <div className="bg-white p-6 rounded-2xl shadow-md max-w-md mx-auto">
      <h2 className="text-xl font-semibold mb-4">Paso 1/3: VTV</h2>

      <label className="block mb-2 font-medium">Certificado de VTV</label>
      <div className="mb-4">
        <input
          type="file"
